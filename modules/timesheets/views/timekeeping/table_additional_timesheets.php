...
if(!is_admin() && !has_permission('timesheets_additional_timesheets','','view')){
    array_push($where,'AND ('. get_hierarchy_sql('additional_timesheets') . ' or '.get_staff_user_id() .' in (select staffid from '.db_prefix().'timesheets_approval_details where rel_type = "additional_timesheets" and rel_id = '.db_prefix().'timesheets_additional_timesheet.id))');
}
